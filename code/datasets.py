...
class ImageDataset_paper(Dataset):
    def __init__(self, root, mode="train", use_mask=False):
        ...
        self.train_input_files = sorted(glob.glob(os.path.join(root, "autodl-tmp/ppr_train/source" + "/*.tif")))
        self.train_target_files = sorted(
            glob.glob(os.path.join(root, "autodl-tmp/ppr_train/target_" + self.retoucher + "/*.tif")))
        self.train_mask_files = sorted(glob.glob(os.path.join(root, "autodl-tmp/masks_360p" + "/*.png")))

        self.test_input_files = sorted(glob.glob(os.path.join(root, "autodl-tmp/ppr_val/source" + "/*.tif")))
        self.test_target_files = sorted(
            glob.glob(os.path.join(root, "autodl-tmp/ppr_val/target_" + self.retoucher + "/*.tif")))
        self.test_mask_files = sorted(glob.glob(os.path.join(root, "autodl-tmp/ppr_val/masks_360p" + "/*.png")))
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            if len(self.train_input_files) == len(self.train_target_files):
                ...
                if self.use_mask:
                    img_mask = Image.open(os.path.join(self.root, "ppr_train/masks_360p/" + img_name[:-4] + ".png"))
            else:
                split_name = img_name.split('_')
                if len(split_name) == 2:
                    img_exptC = Image.open(
                        os.path.join(self.root, "autodl-tmp/ppr_train/target_" + self.retoucher + '/' + img_name))
                    if self.use_mask:
                        img_mask = Image.open(os.path.join(self.root, "ppr_train/masks_360p/" + img_name[:-4] + ".png"))
                else:
                    img_exptC = Image.open(
                        os.path.join(self.root,
                                     "autodl-tmp/ppr_train/target_" + self.retoucher + '/' + split_name[0] + "_" +
                                     split_name[
                                         1] + ".tif"))
                    if self.use_mask:
                        img_mask = Image.open(
                            os.path.join(self.root,
                                         "autodl-tmp/ppr_train/masks/" + split_name[0] + "_" + split_name[1] + ".png"))
